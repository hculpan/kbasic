...
7 h(i,0)=int(rnd(0)*50)+11:h2(i,1)=int(rnd(0)*20)+1+h(i,0):d$(i)="":next:f(8,h+9)=1000000:f or  i=0 to 11:read c$(i):next:h1=-1:h7=0:f or  i=0 to h:o(i)=i:next
8 h1=h1+1:f or  i=0 to 7:h(i,0)=h(i,0)+int(rnd(0)*10)-4:if h(i,0)<10 then h(i,0)=10
9 h(i,1)=h(i,1)+int(rnd(0)*3)-1:if h(i,1)<h(i,0)-5 then h(i,1)=h(i,0)-5
10 d$(i)=d$(i)+chr$(h(i,0)):next:f or  o=0 to h:i=o(o):gosub 5000:k(0)=int(rnd(0)*4):k(1)=int(rnd(0)*4)+4:k(2)=int(rnd(0)*4)+8:gosub 4100:f or  l=0 to 2:m=k(l):j=int(e(i,0)/h(m,0)):if h(m,1)<h(m,0) then h5=h+1: else h5=h+6
...
17 print "would you like  to  see your net w or th (y/n)?":gosub 4300:if a$="n" then 20: else gosub 4000:print "statement of net w or th f or   ";a$(i):print :print "asset";tab(25);"value":print :print "checking account";tab(25);:print using a4$;e(i,0)
18 print "savings account";tab(25);:print using a4$;e(i,1):print "  net liquid assets";tab(25);:print using a4$;e(i,0)+e(i,1):print :e1=0:f or  j=0 to 7:e1=e1+f(j,i)*h(j,0):next:print "c or p or ate s to ck";tab(25);:print using a4$;e1:e2=0:f or  j=8 to 11:e2=e2+f(j,i)*h(j,0):next
...
21 gosub 4000:gosub 3900:f or  j=0 to 7:print j+1;"  ";b$(j);tab(27);h(j,0);tab(32);f(j,i):next:print :print "which s to ck #?":getkey a$:if a$=chr$(13) then 14:if a$<"1" then 21:a$>"8" then 21: else k=val(a$)-1:l=int(f(h2,h+9)/h(k,0)):if l>f(k,i) then l=f(k,i)
22 if l=0 then 14: else print ;"you may sell";l;"share(s) f or   $";h(k,0);"@."
...
24 if h5=0 then 28: else print "do you wish  to  sell any holding company s to ck (y/n)?":gosub 4300:if a$="n" then 28: else gosub 4000:gosub 3900:f or  j=8 to 11:print j-7;"  ";b$(j),:print using a5$;h(j,0),:print ,f(j,i):next:print :print "which s to ck #?"
...
26 print "you may sell";l;"shares f or   $";h(k,0);"@."
...
28 next:f or  i=0 to 11:if f(i,h+8)<>h1 then 36: else gosub 4000:print "this is the s to ckholders' meeting f or   ";b$(i):print :print "candidate";tab(26);"# shares":print :f or  j=0 to h+1:j(j)=f(i,j):next:f or  j=8 to 11:j(f(j,h+7))=j(f(j,h+7))+f(i,h+j-6):next:f or  j=0 to h+1
29 if j(j)>0print a$(j);tab(25);:print using a1$;j(j)
30 next:print :print "the current chairman is ";a$(f(i,h+7));".":k=0:l=0:m=0:f or  j=0 to h+1:if j(j)>k then k=j(j):l=j:m=0: else if j(j)=k then m=1
...
37 if (h1+1)/6<>int((h1+1)/6)goto 8: else f or  i=0 to h:o(i)=o(i)+1:if o(i)>h then o(i)=0
40 next:gosub 4000:print "quarterly dividend announcements":print :print "company";tab(27);"$/share";tab(40);"dividend/share":print :f or  j=0 to 7:f(j,h+9)=(h(j,0)*5+rnd(100))/100:print b$(j);tab(25);:print using a5$;h(j,0),
41 print tab(40);:print using a3$;f(j,h+9):next:f or  i=0 to 7:f or  j=0 to h:e(j,0)=e(j,0)+f(i,j)*f(i,h+9):e(j,5)=e(j,5)+f(i,j)*f(i,h+9):next:f or  j=8 to 11:f(j,h+9)=f(j,h+9)+f(i,h+j-6)*f(i,h+9):next:if f(i,h+9)<4 then f(i,h+6)=f(i,h+6)+10000
42 next:f or  i=8 to 11:f(i,h+9)=f(i,h+9)-10500:f=10000-f(i,h+6):e=f(i,h+9):f or  j=0 to 11:e=e+f(j,h+i-6)*h(j,0):next:h(i,1)=int(e/f):next:gosub 4200:gosub 4000:print "holding company s to ck price change announcement":print 
43 print "holding company       old price    new price    outstanding":print tab(48);"shares":print :f or  i=8 to 11:print b$(i);tab(23);:print using a5$;h(i,0),:print tab(36);:print using a5$;h(i,1),:print ,10000-f(i,h+6):h(i,0)=h(i,1):h(i,1)=h(i,1)+rnd(19)-10:next
44 gosub 4200:if h1<23 then 8: else f or  i=0 to h:gosub 4000:print "yearly income tax f or   ";a$(i):print :print "income type";tab(30);"amount";tab(45);"taxable amount":print :print "earned";tab(20);:print using a4$;e(i,4),:e(i,4)=e(i,4)-5000:if e(i,4)<0 then e(i,4)=0
...
48 print tab(43);:print using a4$;e(i,6):e=0:f or  j=3 to 6:e=e+e(i,j):e(i,j)=0:next:print :print "taxable income";tab(43);:print using a4$;e:e=int(e*25)/100:print :print "income tax";tab(43);:print using a4$;e:e(i,0)=e(i,0)-e:e(i,2)=e(i,2)+400:gosub 4200:next:h1=-1
49 gosub 4000:print "list of c or p or ate chairmanships":print :f or  i=0 to h+1:j(i)=0:next:f or  i=0 to 7:k=f(i,h+7):print b$(i);tab(35);a$(k):j(k)=j(k)+1:next:gosub 4200:gosub 4000:print "list of holding company chairmanships":print :f or  i=8 to 11:k=f(i,h+7):j(k)=j(k)+3
50 print b$(i);tab(35);a$(k):next:print :k=0:f or  i=0 to h:if j(i)>10 then print a$(i);" is the winner of the game.":print :end: else next:gosub 4200
51 gosub 4000:print " #";tab(10);"company":print :f or  i=0 to 7:print i+1;tab(10);b$(i):next:print @165,"rec or ds have been kept of";:print ,"the c or p or ate s to ck prices";:print @293,"over the last";h7+1;"years.";:print @357,"these rec or ds are shown";
52 print ,"graphically.";:print @832,"which company #?"
53 a=inkey$:if a="" then 53: else if a=chr$(13) then 57: else if a<"1" or a>"8" then 53: else i=val(a)-1:gosub 4000:print "the s to ck price rec or d of ";b$(i);" f or  ";h7+1;"year(s).":print @320,"$140":print @448,"$110":print @576," $80":print @704," $50":print ," $20"
54 print ,"      jfmamjjasond  jfmamjjasond  jfmamjjasond  jfmamjjasond";:l=12:f or  j=1 to 24*(1+h7):k=43-int((asc(mid$(d(i),j,1))+1)/5):if k<9 then k=9
55 f or  m=k to 43:set(l,m):next:l=l+1:if j/24=int(j/24) then l=l+4
56 next:gosub 4200:goto 51
57 h7=h7+1:if h7<4 then 8: else h7=h7-1:f or  i=0 to 7:d(i)=right$(d(i),72):next:goto 8
...
3800 f or  j=0 to 11:if f(j,h+1)=0 then h(j,1)=25000
3810 next:return
3900 print " #   company";tab(27);"$/sh";tab(32);"owned":print :return
4000 scnclr:print "-*- wall street & c or p or ate control -*-":print :return
4100 gosub 4000:print "currently listed on the nyse:":print :gosub 3900:f or  n=0 to 2:print n+1"  ";b$(k(n));tab(25);h(k(n),0);tab(32);f(k(n),i):next:print :return
4200 getkey b$
