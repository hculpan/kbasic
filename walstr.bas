...
12 if j>0 then print "you may buy";j;"share(s) of ";b$(m);" stock.":input"how many shares do you wish to buy";k:if k>j or k<0 or k<>int(k) then 12: else f(m,i)=f(m,i)+k:f(m,h5)=f(m,h5)-k:e(i,0)=e(i,0)-k*h(m,0):gosub 4100:if m>7 and h5<>h+1 then f(m,h+9)=f(m,h+9)+k*h(m,0)
...
19 print "holding company stock";tab(25);:print using a4$;e2:print "  net stock assets";tab(25);:print using a4$;e1+e2:print :print "  net w or th";tab(25);:print using a4$;e1+e2+e(i,0)+e(i,1):gosub 4200:goto 14
20 if h4=0 then 24: else a$="":print "do you wish to sell any corporate stock (y/n)?":gosub 4300:if a$="n" then 24
21 gosub 4000:gosub 3900:for  j=0 to 7:print j+1;"  ";b$(j);tab(27);h(j,0);tab(32);f(j,i):next:print :print "which stock #?":getkey a$:if a$=chr$(13) then 14:if a$<"1" then 21:a$>"8" then 21: else k=val(a$)-1:l=int(f(h2,h+9)/h(k,0)):if l>f(k,i) then l=f(k,i)
