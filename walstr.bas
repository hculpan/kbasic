...
4 clear:gosub 4000:print "how many players (2/6)? ";:a1$="#,###,###":a2$="$#,#######.##":a3$="$,####.##":a4$="$###,#######.##":a5$="#$###":a9$(0)="not ":a9$(1)=""
...
6 next:for  i=0 to 7:read b$(i):f(i,h+1)=2500:f(i,h+6)=97500:f(i,h+7)=h+1:f(i,h+8)=int(rnd(0)*24):next:print :for  i=0 to h:print "name of player #";i+1;:input a$(i):e(i,0)=1000:e(i,1)=25000:e(i,2)=500:next:a$(h+1)="J.Q. Smith":h(8,0)=1000:h(8,1)=950+int(rnd(0)*200)+1:for  i=0 to 7
...
12 if j<0 then j=0:print "you may buy ";j;" share(s) of ";b$(m);" stock.":input"how many shares do you wish to buy";k:if k>j or k<0 or k<>int(k) then 12: else f(m,i)=f(m,i)+k:f(m,h5)=f(m,h5)-k:e(i,0)=e(i,0)-k*h(m,0):gosub 4100:if m>7 and h5<>h+1 then f(m,h+9)=f(m,h+9)+k*h(m,0)
13 next:gosub 4200:h2=int(rnd(0)*4)+8:h3=8:gosub 3800
14 gosub 4000:print "if you wish to sell some of your stock:":print :if f(h2,h+9)<0 then h4=0: else h4=1
15 if f(h3,h+9)<0 then h5=0: else h5=1
16 print b$(h2);" will ";a9$(h4);"buy any corporate stock.":print b$(h3);" will ";a9$(h5);"buy any holding company stock.":print 
17 print "Would you like to see your net worth (y/n)?":gosub 4300:if a$="n" then 20: else gosub 4000:print "statement of net worth for   ";a$(i):print :print "asset";tab(25);"value":print :print "Checking account";tab(25);:print using a4$;e(i,0)
18 print "Savings account";tab(25);:print using a4$;e(i,1):print "  Net liquid assets";tab(25);:print using a4$;e(i,0)+e(i,1):print :e1=0:for  j=0 to 7:e1=e1+f(j,i)*h(j,0):next:print "Corporate stock";tab(25);:print using a4$;e1:e2=0:for  j=8 to 11:e2=e2+f(j,i)*h(j,0):next
19 print "Holding company stock";tab(25);:print using a4$;e2:print "  Net stock assets";tab(25);:print using a4$;e1+e2:print :print "  Net worth";tab(25);:print using a4$;e1+e2+e(i,0)+e(i,1):gosub 4200:goto 14
20 if h4=0 then 24: else a$="":print "Do you wish to sell any corporate stock (y/n)?":gosub 4300:if a$="n" then 24
21 gosub 4000:gosub 3900:for  j=0 to 7:print j+1;"  ";b$(j);tab(27);h(j,0);tab(32);f(j,i):next:print :print "which stock #?":getkey a$:if a$=chr$(13) then 14:if a$<"1" then 21:if a$>"8" then 21: else k=val(a$)-1:l=int(f(h2,h+9)/h(k,0)):if l>f(k,i) then l=f(k,i)
...
24 if h5=0 then 28: else print "do you wish to sell any holding company stock (y/n)?":gosub 4300:if a$="n" then 28: else gosub 4000:gosub 3900:for  j=8 to 11:print j-7;"  ";b$(j),:print using a5$;h(j,0),:print ,f(j,i):next:print :print "which stock #?"
...
28 next:for  i=0 to 11:if f(i,h+8)<>h1 then 36: else gosub 4000:print "this is the stockholders' meeting for   ";b$(i):print :print "candidate";tab(26);"# shares":print :for  j=0 to h+1:j(j)=f(i,j):next:for  j=8 to 11:j(f(j,h+7))=j(f(j,h+7))+f(i,h+j-6):next:for  j=0 to h+1
29 if j(j)>0 then print a$(j);tab(25);:print using a1$;j(j)
...
31 next:k=f(i,h+7):if m=1 then print "the chairman has been re-elected as a compromise candidate.":goto 35: else if l=k then print "the chairman has been re-elected.":goto 35: else print a$(l);" has been elected as the new chairman.":f(i,h+7)=l
...
35 gosub 4200
36 next:if f(8,h+2)>0 then f(8,h+6)=f(8,h+6)+f(8,h+2):f(8,h+2)=0
37 if (h1+1)/6<>int((h1+1)/6) then goto 8: else for  i=0 to h:o(i)=o(i)+1:if o(i)>h then o(i)=0
...
42 next:for  i=8 to 11:f(i,h+9)=f(i,h+9)-10500:f=10000-f(i,h+6):e=f(i,h+9):for  j=0 to 11:e=e+f(j,h+i-6)*h(j,0):next:h(i,1)=int(e/f):next:gosub 4200:gosub 4000:print "holding company stock price change announcement":print 
...
47 print tab(43);:print using a4$;e(i,5):print "stock sales";tab(20);:print using a4$;e(i,6),:e(i,6)=e(i,6)/4-25000:if e(i,6)<0 then e(i,6)=0
...
51 gosub 4000:print " #";tab(10);"company":print :for  i=0 to 7:print i+1;tab(10);b$(i):next:print @165,"rec or ds have been kept of";:print ,"the corporate stock prices";:print @293,"over the last";h7+1;"years.";:print @357,"these rec or ds are shown";
...
55 for m=k to 43:dummy$="set(l,m)":next:l=l+1:if j/24=int(j/24) then l=l+4
56 next:gosub 4200:goto 51
57 h7=h7+1:if h7<4 then 8: else h7=h7-1:for i=0 to 7:d(i)=right$(d(i),72):next:goto 8
1010 data "Tanstaafl Holdings","American Holdings","United Holdings","Evergreen Holdings","Bellevue square","Northgate Mall","Weyhaeuser Corp.","Georgia-Pacific","Seattle 1st Ntl Bank","Rainier ntl. Bank","Boeing Aerospace","Pacific Car & Fndry."
1020 data "January","February","March","April","May","June","July","August","September","October","November","December"
...
4220 if b$=chr$(13) then return: else goto 4200
4300 a$="":getkey a$
...
5010 print " half of ";c$(int(h1/2));".":print :j=int(e(i,1)/4)/100:e(i,1)=e(i,1)+j:e(i,3)=e(i,3)+j:print "Interest income:";tab(25);:print using a2$;j:print "Earned income:";tab(25);:print using a2$;e(i,2):e(i,4)=e(i,4)+e(i,2):e(i,0)=e(i,0)+e(i,2)
